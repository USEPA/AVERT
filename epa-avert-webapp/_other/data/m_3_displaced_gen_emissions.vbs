...
For a = 1 To 5          'loops through sheets
    ...
    For b = 1 To loadArray_length
        ...
        If loadArray(b, 1) >= minBin And loadArray(b, 1) <= maxBin And loadArray(b, 3) >= minBin And loadArray(b, 3) <= maxBin Then
            ...
            block1A = matchedArray(b, 1)
            block2A = block1A + 1

            block1B = matchedArray(b, 2)
            block2B = block1B + 1

            For c = 1 To lastRowColJ - 4
                
                'pre
                slopeA = (plantDataArray(c, block1A) - plantDataArray(c, block2A)) / (genBlockArray(1, block1A) - genBlockArray(1, block2A))
                interceptA = plantDataArray(c, block1A) - slopeA * genBlockArray(1, block1A)
                valA = loadArray(b, 1) * slopeA + interceptA
                
                'post
                slopeB = (plantDataArray(c, block1B) - plantDataArray(c, block2B)) / (genBlockArray(1, block1B) - genBlockArray(1, block2B))
                interceptB = plantDataArray(c, block1B) - slopeB * genBlockArray(1, block1B)
                valB = loadArray(b, 3) * slopeB + interceptB
