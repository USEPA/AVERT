...
/**
 * fipsCodes was created from the 'CountyFIPS' sheet of the `AVERT Main
 * Module v2.0 (20180201).xlsm` file. If changes are made to the sheet in Excel,
 * this variable needs to be updated, with special attention applied to:
 * - `code`:
 *     ensure they're strings and retain a leading zero (if applicable)
 * - `county`:
 *     ensure cities are displayed with the word city capitalized in parenthesis
 *     e.g. `Baltimore (City)`
 * - **NOTE:** special formatting for "La Porte county, Indiana": the web app
 *   displays it as 'LaPorte', so use that exact formatting
 */
export const fipsCodes: { code: string; state: string; county: string }[] = [
  {
    code: '01001',
    state: 'Alabama',
    county: 'Autauga',
