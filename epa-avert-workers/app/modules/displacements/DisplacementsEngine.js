...
const DisplacementsEngine = (function () {
    ...
    DisplacementsEngine.prototype.getDisplacedGeneration = function (dataSet, dataSetNonOzone, emissionType) {
        ...
        let postTotalArray = [];
        let deltaVArray = [];
        const monthlyEmissions = {regional: {data: {}, pre: {}, post: {}}, state: {}};
        ...
        const mediansNonOzone = dataSetNonOzone ? dataSetNonOzone.map(function (item) {
            return item.medians
        }) : false;
        ...
        for (let i = 0; i < loadArrayOriginal.length; i ++) {
            ...
            monthlyPreValues.region[month] = monthlyPreValues.region[month] ? monthlyPreValues.region[month] : 0;
            monthlyPostValues.region[month] = monthlyPostValues.region[month] ? monthlyPostValues.region[month] : 0;
    ...
    DisplacementsEngine.prototype.calculateMedians = function calculateMedians (medians, index, data, load, preIndex, edges, postLoad, postIndex, monthlyEmissions, month) {
        // Calculate medians
    ...
    DisplacementsEngine.prototype.excelMatch = function excelMatch (array, lookup) {

        const sortedHaystack = array.concat(lookup).sort(function sort (a, b) {
            return a - b;
        });
    ...
    DisplacementsEngine.prototype.getDisplacedLoadArray = function getDisplacedLoadArray (loadArrayOriginal, hourlyEere) {
        return loadArrayOriginal.map(function (item, index) {
            return math.sum(item, hourlyEere[index].final_mw)
        });
    };

    DisplacementsEngine.prototype.useOzoneOrNon = function useOzoneOrNon (medians, mediansNonOzone, month) {
