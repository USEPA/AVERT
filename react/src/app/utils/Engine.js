...
class Engine {
	...
	calculateEERE() {
		...
		const hourly_eere = _.map(regionalLoad, function(load, index) {
			...
			let firstMaths = (load > top_percentile) ? load * (-_this.profile.reduction) : 0
			let secondMaths = math.chain(firstMaths)
								...
								.done();
			let finalMw = data.regional_load[index].year < 1990 ? 0 : secondMaths;
